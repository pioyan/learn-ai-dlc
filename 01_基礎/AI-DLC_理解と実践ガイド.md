...
- Intent: 目的・背景・成功基準をまとめた最上位ドキュメント
- Unit: Intentを価値単位へ分解した独立作業単位
- Bolt: 数時間〜1日単位で回す最小反復
...
- フェーズ間の成果物連結（上流から下流まで追跡できる状態）を明示する
...
DORA研究は、開発能力と組織成果の関係を継続的に示しており、AI時代でも「デリバリーフロー（変更頻度やリードタイム）と安定性」の同時改善が重要です。Thoughtworks Technology Radar（2026年版）でも、AI時代ほどDORA指標の重要性が増すと整理されています。
...
### 5.4 導入技法: Context Engineering（AIに必要な文脈を必要なタイミングで渡す設計）を最初に整える
...
- 長い作業では中間成果を要約し、AIに渡す文脈の品質低下を防ぐ

AI-DLCで言えば、Intent→Unit→Design→Code→Opsの成果物連結そのものが、実務的なContext Engineeringの実装になります。
...
- 2チーム目へ展開
- エージェント共通指示（agent instructions）とテンプレートをサービス雛形に埋め込み
- 監査ログと運用手順を整備
- 四半期単位のROI評価を開始
